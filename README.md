...
```
┌──────────────────────────────────────────────────────────┐
│ 拾址 / SHIZHI   9月9日 · 收录 42 条 · 常用 5 · 今日已开 3 │  ← 刊头即标题栏，可拖拽
├──────────────────────────────────────────────────────────┤
│  🔍  ⊕  ⚙  ☾  ⋮                                − □ ×    │  ← 检索 / 新增 / 分类 / 主题 / 菜单
├──────────────────────────────────────────────────────────┤
│  ┌──────────┐ ┌──────────┐ ┌──────────┐                   │
...
| **不做** | 多用户、云同步、账号、嵌套文件夹、浏览器插件商店化 |

### 设计取向

杂志编辑风负责**性格与记忆点**（刊头、`No.07` 编号、1px 细线、衬线大字、朱红点缀），
速查台负责**效率**（一个搜索框、一次点击、常用置顶）。

刻意避开的 AI 味：不用圆角大阴影堆卡片，不用紫蓝渐变，不用 Inter/Roboto。
方形细线 + 暖纸底色 + 朱红，中文标题落宋体，西文落 Palatino/Georgia——**全部本地字体，零网络请求**。
